...
class Net_Pose_HG(nn.Module):
    def __init__(self, num_joints, num_stages=2, num_modules=2, num_feats=256):
        ...
        self.pose2d_conv = nn.Sequential(
            ...
            nn.BatchNorm2d(512), self.relu,
            nn.Conv2d(512, 512, bias=True, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(512), self.relu,
            nn.Conv2d(512, 1024, bias=True, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(1024), self.relu,
            nn.Conv2d(1024, 1024, bias=True, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(1024), self.relu, 
            self.avgpool
        )
...
if __name__ == "__main__":
    ...
    net = Net_Pose_HG(10)
    out, coord2d_hm, coord2d_pose = net(img)
    print(len(out),out[0].shape, coord2d_hm.shape, coord2d_pose.shape)
